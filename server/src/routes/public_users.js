...
public_users.post("/register", async (req, res) => {
  const { username, email, password, password_confirm, date_of_birth } = await req.body;
  ...
  if (
    ...
    try {
      ...
      try {
        ...
        return res.status(201).json({ status: 201, message: "You've successfully created an account!, you can now log in!" })
